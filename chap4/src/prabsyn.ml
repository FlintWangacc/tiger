...
let print_field d { A.name; escape; ty; pos } =
  indent d ^ "(" ^ Symbol.name name ^ "," ^ string_of_bool !escape ^ ","
  ^ Symbol.name ty ^ ")"

let rec print_var d = function
  | A.SimpleVar (s, p) -> indent d ^ "SimpleVar(" ^ Symbol.name s ^ ")"
  | A.FieldVar (v, s, p) ->
      indent d ^ "\nFieldVar("
      ^ print_var (d + 1) v
      ^ "\n,"
...
and print_exp d = function
  ...
  | A.NilExp -> indent d ^ "NilExp"
  | A.IntExp i -> indent d ^ "IntExp(" ^ string_of_int i ^ ")"
  | A.StringExp (s, p) -> indent d ^ "StringExp(\"" ^ s ^ "\")"
  | A.CallExp (s, args, p) ->
...
and print_dec d = function
  | A.FunctionDec l -> indent d ^ "FunctionDec[" ^ dolist d print_fundec l ^ "]"
  | A.VarDec
      { A.vardec_name; vardec_escape; vardec_ty; vardec_init; vardec_pos } ->
      indent d ^ "VarDec(" ^ Symbol.name vardec_name ^ ")"
      ^ string_of_bool !vardec_escape
      ^ ","
      ^ (match vardec_ty with
        | Some (s, _) -> "Some(" ^ Symbol.name s ^ ")"
        | None -> "None")
      ^ ")" ^ "\n,"
      ^ print_exp (d + 1) vardec_init
      ^ ")"
  | A.TypeDec l -> indent d ^ "TypeDec[" ^ dolist d print_tydec l ^ "]"

and print_ty d = function
  | A.NameTy (s, p) -> indent d ^ "NameTy(" ^ Symbol.name s ^ ")"
  | A.RecordTy fields ->
      indent d ^ "RecordTy[" ^ dolist d print_field fields ^ "]"
  | A.ArrayTy (s, p) -> indent d ^ "ArrayTy(" ^ Symbol.name s ^ ")"

and print_fundec d
    { A.fundec_name; fundec_params; fundec_result; fundec_body; fundec_pos } =
  indent d ^ "(" ^ Symbol.name fundec_name ^ ",["
